...
export default function AnalisisIA({ onResultado, imagenActual }) {
  ...
  const handleAnalizar = async () => {
    ...
    try {
      ...
      if (res.diagnostico_principal) {
        ...
        if (modoAhorro) {
          try {
            await liberarModelo('biomedclip');
          } catch (e) {
  ...
  // Obtener estado del modelo BiomedCLIP
  const getEstadoBiomedclip = () => {
    if (!estadoModelo) return null;
    // Soportar tanto el formato antiguo como el nuevo
    if (estadoModelo.biomedclip) return estadoModelo.biomedclip;
    return estadoModelo; // Formato antiguo
  };

  const estadoBiomedclip = getEstadoBiomedclip();
        ...
        {estadoBiomedclip && (
          <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className={`font-medium ${estadoBiomedclip.cargado ? 'text-green-600' : 'text-gray-500'}`}>
                {estadoBiomedclip.cargado ? 'Cargado' : 'No cargado'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <HardDrive className="w-4 h-4 text-gray-500" />
              <span className="text-gray-600">RAM:</span>
              <span className="font-medium text-gray-700">{estadoBiomedclip.consumo_ram || '~1.5 GB cuando está cargado'}</span>
            </div>
            {estadoBiomedclip.cargado && (
              <button
